...
void pwm_init(TIMx_TypeDef * TIMx){
    TIMx->PSC = 9;                      // Setting prescaler to 99 to get CK_CNT = 10k Hz from 1 MHz clock input
...
void delay_init(TIMx_TypeDef * TIMx){
   TIMx->PSC = 9;                    //(CK_CNT / 1000) - 1;//(CK_CNT / 1000) - 1; // Creating one ms resolution from system clock
...
void delay_update(TIMx_TypeDef * TIMx, int duration){
   TIMx->ARR = duration * 100;    // With a 100KHz clock, each tick is 10 milliseconds. So the freqeuncy should be multiplied by 10 with 1 subtracted from it
   TIMx->EGR |= (1<<0);  
